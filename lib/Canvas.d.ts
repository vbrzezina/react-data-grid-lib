...
export default class Canvas extends React.PureComponent<Props> {
    ...
    getRows(rowOverscanStartIdx: number, rowOverscanEndIdx: number): {
        row: RowData;
        subRowDetails: SubRowDetails<any> | undefined;
    }[];
    getScroll(): {
        scrollTop: number;
        scrollLeft: number;
    ...
    getRowByRef: (i: number) => (RowRenderer & React.Component<RowRendererProps<any, any>, {}, any>) | undefined;
    getRowTop: (rowIdx: number) => number;
    getRowHeight: (rowIdx: number) => number;
    getRowColumns: (rowIdx: number) => CalculatedColumn<any, any>[];
